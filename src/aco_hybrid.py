...
class ACO_Hybrid(ACO_Base):
    ...
    def __init__(self, city_coords, model_path="tsp_model.pth", gamma=1.5, **kwargs):
        ...
        with torch.no_grad():
            for i in range(self.n_cities):
                for j in range(self.n_cities):
                    if i != j:
                        feat = self._build_features(i, j,
                                                    partial_len=0.0,
                                                    visited_ratio=0.0)
                        #scores[i][j] = self.sigmoid(self.model(feat)).item()
    ...
    def _build_features(self, i, j, partial_len, visited_ratio):
        ...
        return feat.unsqueeze(0)  # shape: (1, 5)
                ...
                for j in range(self.n_cities):
                    if j not in path:
                        feat = self._build_features(i, j,
                                                    partial_len,
                                                    visited_ratio)
                        self.nn_scores[i][j] = torch.sigmoid(
                            self.model(feat)
                        ).item()
    ...
    def _calculate_probabilities_nn(self, i, visited):
        ...
        weights = (tau ** self.alpha) * (eta ** self.beta) * np.exp(self.gamma * q)
        total = weights.sum()
